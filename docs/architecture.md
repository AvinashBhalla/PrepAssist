...
## Deterministic coverage and schedule validation

`checkCoverage(requirements, questions)` is pure application code. A requirement is covered only when a question explicitly contains its existing ID in `requirement_ids`; no semantic similarity, keyword matching, embedding, or LLM judgment is used. Must requirements populate `uncovered_requirement_ids` in requirement order, nice requirements remain diagnostic-only, and unknown question references are reported without creating phantom requirements. The checker returns facts and does not own `coverage.passes`; later orchestration controls pass accounting.

Question priority uses the documented formula `mustCount * 1000 + niceCount * 100 + difficulty * 10`. Questions are sorted by descending score and then ascending question ID. Difficulty duration is deterministic: difficulty 1 = 10 minutes, 2 = 15 minutes, and 3 = 25 minutes.

`allocateSchedule` creates exactly the requested number of sequential days, rejects non-positive or fractional days, and supports up to 3650 days without silently clamping. Questions are placed once in sorted priority order into the least-loaded day within an expanding early-day prefix; this favors must coverage and difficulty early while balancing minutes and allowing unused days with zero minutes. Focus strings are derived from assigned requirement text or categories, never generated by an LLM. Every scheduled ID comes from the supplied question list, and all supplied questions are scheduled, so existing must coverage cannot be lost through placement.

`validateFinalKit` first validates the exact Appendix A sections and field enums, then performs deterministic cross-reference checks: unique requirement/question/flashcard IDs, valid requirement and question references, exact schedule day count and numbering, integer minutes, positive coverage passes, deterministic coverage agreement, complete must coverage, and scheduled must-question coverage. It returns structured errors with codes, paths, and messages rather than generic exceptions. These decisions remain separate from model generation so final contract correctness does not depend on LLM behavior.
